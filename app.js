...
async function handleSetup() {
  ...
  try {
    const data = await callBackend("setupClient", { businessName, phone, address });
...
async function loadDailySummary() {
  const salesEl = $("statSalesValue");
  const expensesEl = $("statExpensesValue");
  const collectionsEl = $("statCollectionsValue");

  [salesEl, expensesEl, collectionsEl].forEach((el) => {
  ...
  try {
    ...
    salesEl.textContent = String(data.salesToday);
    expensesEl.textContent = "BDT " + data.expensesToday;
    collectionsEl.textContent = "BDT " + data.collectionsToday;
  } catch (err) {
    salesEl.textContent = "—";
    expensesEl.textContent = "—";
    collectionsEl.textContent = "—";
  } finally {
    [salesEl, expensesEl, collectionsEl].forEach((el) => el.classList.remove("shimmer"));
  }
}
 ...
 * always returns { name, size, price, rowIndex } now, but this
 * keeps things safe if a plain string ever slips through.
 */
function normalizeProduct(product) {
  if (typeof product === "string") {
    return { name: product, size: "", price: "", rowIndex: null };
  }
  return {
...
async function handleAddOrUpdateProduct() {
  ...
  try {
    if (editingProductRowIndex) {
      await callBackend("updateProduct", { sheetId, rowIndex: editingProductRowIndex, name, size, price });
      showMessage(msgEl, "Product updated!", "success");
    } else {
      await callBackend("addProduct", { sheetId, name, size, price });
