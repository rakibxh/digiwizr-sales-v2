...
const translations = {
  ...
  costOfGoodsSold: { en: "Cost of Goods Sold", bn: "বিক্রিত পণ্যের সর্বমোট ক্রয়মূল্য" },
  grossProfit: { en: "Gross Profit", bn: "সর্বমোট লাভ" },
  profitPercentage: { en: "Profit Percentage", bn: "সর্বমোট লাভের হার" },
  ...
  // SETUP SCREEN
  setupWelcome: { en: "Welcome to Digiwizr Sales Tracker", bn: "Digiwizr সেলস ট্র্যাকার-এ স্বাগতম" },
  ...
  businessAddress: { en: "Business Address (optional)", bn: "ব্যবসার ঠিকানা (ঐচ্ছিক)" },
  createPin: { en: "Create a 4-digit PIN — you will enter this every time you open the app", bn: "৪-সংখ্যার পিন তৈরি করুন - প্রতিবার অ্যাপে প্রবেশ করতে দরকার হবে" },
  confirmPin: { en: "Confirm PIN", bn: "পিন নিশ্চিত করুন" },
  setupAccountBtn: { en: "Set Up My Account", bn: "অ্যাকাউন্ট সেটআপ করুন" },
  recoverLinkFromSetup: { en: "Already have an account? Recover it here", bn: "ইতিমধ্যেই একটি অ্যাকাউন্ট আছে? এখানে পুনরুদ্ধার করুন" },
  ...
  errPinLength: { en: "PIN must be exactly 4 digits.", bn: "পিন অবশ্যই ৪-সংখ্যার হতে হবে।" },
  errPinMismatch: { en: "PINs do not match.", bn: "পিন নম্বর ম্যাচ করেনি।" },

  // PIN ENTRY SCREEN
  pinEntryLabel: { en: "Enter your 4-digit PIN", bn: "৪-সংখ্যার পিন প্রবেশ করান" },
  unlockBtn: { en: "Unlock", bn: "আনলক করুন" },
  forgotPinLink: { en: "Forgot PIN? Recover account", bn: "পিন ভুলে গেছেন? অ্যাকাউন্ট পুনরুদ্ধার করুন" },
  incorrectPin: { en: "Incorrect PIN. Please try again.", bn: "ভুল পিন। অনুগ্রহ করে আবার চেষ্টা করুন।" },

  // RECOVERY SCREEN
  recoveryTitle: { en: "Account Recovery", bn: "অ্যাকাউন্ট পুনরুদ্ধার" },
  recoveryPinLabel: { en: "Enter your 6-digit Recovery PIN", bn: "আপনার ৬-সংখ্যার পুনরুদ্ধার পিন প্রবেশ করান" },
  recoverAccountBtn: { en: "Recover Account", bn: "অ্যাকাউন্ট পুনরুদ্ধার করুন" },
  setNewPinTitle: { en: "Set a New PIN", bn: "নতুন পিন সেট করুন" },
  recoverySubtext: { en: "Your account was found. Choose a new 4-digit PIN for this device.", bn: "আপনার অ্যাকাউন্ট পাওয়া গিয়েছে। এই ডিভাইসের জন্যে নতুন ৪-সংখ্যার পিন সেট করুন।" },
  newPinLabel: { en: "New PIN", bn: "নতুন পিন" },
  confirmNewPinLabel: { en: "Confirm New PIN", bn: "নতুন পিন নিশ্চিত করুন" },
  setNewPinBtn: { en: "Set New PIN", bn: "নতুন পিন সেট করুন" },
  recoveryNotFound: { en: "Recovery PIN not found. Please check and try again.", bn: "পুনরুদ্ধার পিন পাওয়া যায়নি। অনুগ্রহ করে আবার চেষ্টা করুন।" },
};
...
async function handleSetup() {
  ...
  if (!businessName) {
    showMessage(msgEl, t("errBusinessName"), "error");
    return;
  }
  if (!phone) {
    showMessage(msgEl, t("errPhone"), "error");
    return;
  }
  if (!email) {
    showMessage(msgEl, t("errEmail"), "error");
    return;
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    showMessage(msgEl, t("errEmailInvalid"), "error");
    return;
  }
  if (!/^\d{4}$/.test(pin)) {
    showMessage(msgEl, t("errPinLength"), "error");
    return;
  }
  if (pin !== pinConfirm) {
    showMessage(msgEl, t("errPinMismatch"), "error");
    return;
  }
...
async function handleRecoverAccount() {
  ...
  try {
    ...
    showMessage(msgEl, t("recoveryNotFound"), "error");
  } finally {
    btn.disabled = false;
  }
}
...
let allSaleProducts = []; // full unfiltered product list for the Sale screen search box
...
function filterSaleProducts() {
  ...
  const filtered = allSaleProducts.filter((rawProduct) => {
    const name = normalizeProduct(rawProduct).name.toLowerCase();
...
/* =========================================================
   ...
   ========================================================= */
let allManageProducts = []; // full unfiltered product list for the Manage Products search box
...
function filterManageProducts() {
  ...
  const filtered = allManageProducts.filter((rawProduct) => {
    const name = normalizeProduct(rawProduct).name.toLowerCase();
    return name.includes(query);
  });
