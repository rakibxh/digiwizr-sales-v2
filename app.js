...
async function loadDailySummary() {
  ...
  try {
    ...
    revenueEl.textContent = "BDT " + revenue;

    profitEl.textContent = "BDT " + profit;
...
function buildProductMetaText(product) {
  ...
  parts.push("Cost: BDT " + product.cost);
  parts.push("Price: BDT " + product.price);
  return " | " + parts.join(" | ");
}
...
async function loadDailyReport(isoDate) {
  ...
  try {
    ...
    showMessage(msgEl, "Could not load report: " + err.message, "error");
  } finally {
    loadingEl.style.display = "none";
  }
}
...
function renderReportSales(sales) {
  ...
  if (sales.length === 0) {
    el.innerHTML = '<div class="report-empty-state">No sales recorded for this date.</div>';
    return;
  }
  ...
  const rows = sales.map((s) => {
    ...
    return `
      <div class="report-table-row">
        ...
        <span>${qty}</span>
        <span>BDT ${unitPrice}</span>
        <span>BDT ${roundTo2Display(rowTotal)}</span>
      </div>`;
  }).join("");
  ...
  el.innerHTML = `
    <div class="report-table">
      <div class="report-table-header-row">
        <span>Product</span><span>Size</span><span>Qty</span><span>Unit Price</span><span>Total</span>
      </div>
      ${rows}
      <div class="report-subtotal-row"><span>Total Sales:</span><span>BDT ${roundTo2Display(total)}</span></div>
    </div>`;
}
...
function renderReportTwoColumnSection(elId, entries, emptyText, subtotalLabel) {
  ...
  el.innerHTML = `
    <div class="report-table">
      ${rows}
      <div class="report-subtotal-row"><span>${subtotalLabel}</span><span>BDT ${roundTo2Display(total)}</span></div>
    </div>`;
}
...
function renderReportSummary(summary) {
  ...
  $("reportSummaryBody").innerHTML = `
    <div class="report-summary-row"><span>Total Cash In</span><span class="value">BDT ${roundTo2Display(totalCashIn)}</span></div>
    <div class="report-summary-row"><span>Total Expenses</span><span class="value">BDT ${roundTo2Display(totalExpenses)}</span></div>
    <div class="report-summary-row"><span>Cost of Goods Sold</span><span class="value">BDT ${roundTo2Display(costOfGoodsSold)}</span></div>
    <div class="report-summary-row"><span>Gross Profit</span><span class="value ${profitClass}">BDT ${roundTo2Display(grossProfit)}</span></div>
